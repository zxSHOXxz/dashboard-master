...
class PermissionsSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($roles_structure as $role_name => $role_value) {
            ...
            $role = Role::firstOrCreate(['name' => $role_name, 'display_name' => $role_name, 'guard_name' => 'admin']);
            foreach ($role_value as $module => $permission_content) {
                foreach (explode(',', $permission_content) as $p => $perm) {
                    ...
                    $permission = Permission::firstOrCreate(['name' => $module . '-' . $permissions_map[$perm], 'table' => $module, 'guard_name' => 'admin']);
